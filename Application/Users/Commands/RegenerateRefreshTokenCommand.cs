...
{
    public async Task<Result<AuthResult, UserException>> Handle(RegenerateRefreshTokenCommand request, CancellationToken cancellationToken)
    {
        ...
        return await user.Match(
            ...
            async u =>
            {
                ...
                return await refreshToken.Match(
                    ...
                    () => Task.FromResult<Result<AuthResult, UserException>>(new UserRefreshTokenNotFoundException(u.Id, request.Token)));
            },
            () => Task.FromResult<Result<AuthResult, UserException>>(new UserNotFoundException(new UserId(request.UserId))));
    }
    
    private async Task<Result<AuthResult,UserException>> RegenerateToken(User user, RefreshToken token)
    {
        try
        {
